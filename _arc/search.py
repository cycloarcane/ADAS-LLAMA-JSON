...
# Remove the global rate_limiter declaration here
rate_limiter = None
...
if __name__ == "__main__":
    ...
    # Add rate limit arguments
    parser.add_argument('--rate_limit', type=int, default=10, help='Number of requests per minute')
    parser.add_argument('--rate_limit_period', type=int, default=60, help='Rate limit period in seconds')

    args = parser.parse_args()
    
    # Create rate limiter with command-line arguments
    rate_limiter = RateLimiter(args.rate_limit, args.rate_limit_period)
    
    # Apply rate limiter to the API call functions
    get_json_response_from_gpt = rate_limiter(get_json_response_from_gpt)
    get_json_response_from_gpt_reflect = rate_limiter(get_json_response_from_gpt_reflect)
